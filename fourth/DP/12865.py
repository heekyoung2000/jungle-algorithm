...
for i in range(1,n+1):
    for j in range(1,k+1):
        ...
        value = stuff[i][1]
        if j <weight:
           table[i][j] = table[i-1][j] #위에 값 그대로 가져오기
        else:
            print(i,j-weight,j)
            table[i][j] = max(table[i-1][j],table[i][j-weight]+value)
            #table[i-1][j] : 그전 누적 기대값의 최대 값
            #table[i][j-weight] : 물건을 넣은 뒤의 남은 무게를 채울 수 있는 최대값
print(table)       
print(table[n][k]) 

#틀린 이유 : 같은 열에서 계산 하는 table[i][j-weight]를 사용하면 같은 물건을 중복으로 가져 갈 수 있기 때문에 
#table[i-1][j-weight]으로 계산해야 한다.